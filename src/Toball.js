...
import {TableContainer,Table,TableHead,TableRow,TableBody,TablePagination,TableFooter,Menu,MenuItem,TableCell,Checkbox,Button,Toolbar,Typography,Tooltip,Paper,IconButton, Divider} from '@material-ui/core'
...
function Tabla() {
  ...
  const emptyRows = rowsPerPage - Math.min(rowsPerPage, bod.length - page * rowsPerPage);
            ...
            <TableContainer>
                <Table>
                    ...
                    {/*<TableBody>
                        ...
                        <TableBody>
          {(rowsPerPage > 0
            ? bod.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
            : bod
          ).map((row) => (
            <TableRow key={row.name}>
              <TableCell >
              <Checkbox checked={row.check} onChange={()=>chch(bod.indexOf(row))} />
              </TableCell>
              {filter[0].bol&&<TableCell component="th" scope="row">
                {row.name}
              </TableCell>}
              {filter[1].bol&&<TableCell  >
                {row.calories}
              </TableCell>}
              {filter[2].bol&&<TableCell  >
                {row.fat}
              </TableCell>}
              {filter[3].bol&&<TableCell  >
                {row.carbs}
              </TableCell>}
              {filter[4].bol&&<TableCell  >
                {row.protein}
              </TableCell>}
            </TableRow>
          ))}
